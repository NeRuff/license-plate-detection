...
@app.command()
def video(
    input_path: str = typer.Option(..., "--video", "-v", help="Path to video file"),
    output_path: str = typer.Option("./output.mp4", "--output", "-o", help="Output path"),
):
    setup_logging()
...
@app.command()
def camera(
    camera_id: int = typer.Option(0, "--camera", "-c", help="Camera ID"),
):
    setup_logging()
